...
public class CustomTagSearch extends TopicSearchBase implements Serializable {
    ...
    public void downloadXML() {
        ...
        for (final Topic topic : topicList) {
            ...
            } catch (UnsupportedEncodingException e) {                /* UTF-8 is a valid format so this should exception should never
            get thrown */
            }
        }
     ...
     * @param persist true if this filter is being saved to the db (i.e. the user is
    ...
    public String doTextSearch() {
        return "/CustomSearchTopicList.seam?" + CommonFilterConstants.TOPIC_TEXT_SEARCH_FILTER_VAR + "=" + this.topic.getTopicTextSearch
                ().getData();
    }
    ...
    public void generateContentSpec() {
        ...
        if (validateDocbookBuildOptions(this.docbookBuildingOptions)) {
            ...
            HTTPUtilities.writeOutContent(contentSpecs, "ContentSpec-" + dateFormatter.format(new Date()) + ".zip",
