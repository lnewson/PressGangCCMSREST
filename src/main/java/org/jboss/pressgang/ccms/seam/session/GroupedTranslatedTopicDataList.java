...
public class GroupedTranslatedTopicDataList extends GroupedListBase<TranslatedTopicDataList> {
    ...
    public GroupedTranslatedTopicDataList(final String locale, final TranslatedTopicDataFilterQueryBuilder filterQueryBuilder,
            final GroupedTranslatedTopicDataLocaleList parent) {
        this(locale, filterQueryBuilder, parent, null);
    }

    public GroupedTranslatedTopicDataList(final String locale, final TranslatedTopicDataFilterQueryBuilder filterQueryBuilder,
            final GroupedTranslatedTopicDataLocaleList parent, final Integer expectedTotalTranslationCount) {
        this.locale = locale;
    ...
    public void create() {
        ...
        final Filter filter = EntityUtilities.populateFilter(entityManager, urlParameters, CommonFilterConstants.FILTER_ID,
        ...
        if (groupedTags.size() != 0) this.tab = groupedLists.get(0).getGroup();
        else this.tab = Constants.UNGROUPED_RESULTS_TAB_NAME;
    ...
    private List<Integer> buildTagGroups(final TranslatedTopicDataFilterQueryBuilder filterQueryBuilder) {
        ...
        for (final Tag tag : urlGroupedTags) {
            ...
            final TranslatedTopicDataList translatedTopicDataList = new TranslatedTopicDataList(Constants.DEFAULT_ENVERS_PAGING_SIZE,
                    getQuery(condition));
    ...
    private void buildUngroupedResults(final TranslatedTopicDataFilterQueryBuilder filterQueryBuilder, final List<Integer> groupedTags) {
		 ...
		 */
            ...
            TranslatedTopicDataList translatedTopicDataList = new TranslatedTopicDataList(Constants.DEFAULT_ENVERS_PAGING_SIZE,
                    getQuery(condition));
    ...
    public void redirectToZanata() {
        ...
        String link = zanataServerUrl + "/webtrans/Application.html?project=" + zanataProject + "&iteration=" + zanataVersion +
                "&localeId=" + locale;
    ...
    public static Integer buildEstimatedTotalQuery(final Filter filter) {
        ...
        final Join<TranslatedTopicData, TranslatedTopic> translatedTopic = JPAUtils.findJoinedType(includeQuery, TranslatedTopicData.class,
                TranslatedTopic.class);
        if (includeQueryCondtions != null) includeQuery.where(includeQueryCondtions);
        ...
        if (!excludeFields.isEmpty()) {
            ...
            final Join<TranslatedTopicData, TranslatedTopic> exTranslatedTopic = JPAUtils.findJoinedType(excludeQuery,
                    TranslatedTopicData.class, TranslatedTopic.class);
