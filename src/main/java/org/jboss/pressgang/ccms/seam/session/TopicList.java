...
public class TopicList extends EntityQuery<Topic> {
    ...
    private static final String[] RESTRICTIONS = {
            ...
            "topic.topicId in (#{topicList.topic.topicIds})", "lower(topic.topicTitle) like lower(concat('%', " +
            "#{topicList.topic.topicTitle}, '%'))", "lower(topic.topicText) like lower(concat('%', #{topicList.topic.topicDescription}, " +
            "'%'))"};

    protected TopicFieldFilter topic = new TopicFieldFilter();

    public TopicList(final Integer limit, final String constructedEJBQL, final TopicFieldFilter topic, final boolean useRestrictions) {
        setEjbql(constructedEJBQL);

        if (useRestrictions) setRestrictionExpressionStrings(Arrays.asList(RESTRICTIONS));

        /* use the default paging size if no limit is specified */
        if (limit == null) setMaxResults(Constants.DEFAULT_PAGING_SIZE);
        /* if the limit is not -1 (or no paging), set the limit */
        else if (limit != -1) setMaxResults(limit);
