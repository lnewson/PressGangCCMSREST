...
{
    private static final Logger log = LoggerFactory.getLogger(GroupedListBase.class);
    
	/** Points to the EntityQuery that will be used for the paging operations (i.e. the one with the most item) */
	public T pagingEntityQuery;
	...
	{
		if (pagingEntityQuery != null)
			return pagingEntityQuery.getFirstResult() == null ? 0 : pagingEntityQuery.getFirstResult();
		return 0;
	}
	...
	protected List<Tag> getGroupedTags()
	{
		...
		{
			if (urlParam.startsWith(CommonFilterConstants.GROUP_TAG))
			{
				try
				{
					final Integer tagID = Integer.parseInt(urlParam.replace(CommonFilterConstants.GROUP_TAG, ""));
					final Integer state = Integer.parseInt(urlParameters.get(urlParam));
					if (state == CommonFilterConstants.GROUP_TAG_STATE)
					{
				...
				{
					log.error("Probably a bad url parameter passed as a grouping tag", ex);
				}
			}
		}
