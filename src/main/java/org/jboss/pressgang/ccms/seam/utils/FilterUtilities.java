...
public class FilterUtilities extends org.jboss.pressgang.ccms.filter.utils.FilterUtilities {
    ...
    public static void syncFilterWithTags(final Filter filter, final UIProjectsData selectedTags) {
                ...
                for (final UITagData tag : category.getTags()) {
                    ...
                    if (tagSelected) {
                        ...
                        for (final FilterTag filterTag : filter.getFilterTags()) {
                            ...
                            if (tagState == CommonFilterConstants.NOT_MATCH_TAG_STATE || tagState == CommonFilterConstants
                                    .MATCH_TAG_STATE) {
        ...
        for (final FilterTag filterTag : filter.getFilterTags()) {
            ...
            if (!selectedFilterTags.contains(filterTag)) {
                 ...
                 */
                if (found) removeTags.add(filterTag);
            }
        }
    ...
    private static void syncFilterField(final Filter filter, final String fieldName, final String fieldValue,
            final String fieldDescription) {
        // get the database filterfield object that matches the fieldName
        final List<FilterField> filterField = filter(having(on(FilterField.class).getField(), equalTo(fieldName)),
        ...
        if (fieldValue != null && fieldValue.trim().length() != 0) {
            ...
            if (filterField.size() == 1) filter.getFilterFields().remove(filterField.get(0));
        }
    }
    ...
    public static void syncFilterWithCategories(final Filter filter, final UIProjectsData selectedTags) {
        ...
        for (final UIProjectData project : selectedTags.getProjectCategories()) {
            ...
            for (final UICategoryData category : project.getCategories()) {
                ...
                if (intCatLogic.equals(CommonFilterConstants.AND_LOGIC)) dbIntCatLogic = CommonFilterConstants.CATEGORY_INTERNAL_AND_STATE;
                else dbIntCatLogic = CommonFilterConstants.CATEGORY_INTERNAL_OR_STATE;

                if (extCatLogic.equals(CommonFilterConstants.AND_LOGIC)) dbExtCatLogic = CommonFilterConstants.CATEGORY_EXTERNAL_AND_STATE;
                else dbExtCatLogic = CommonFilterConstants.CATEGORY_EXTERNAL_OR_STATE;
                ...
                for (final FilterCategory filterCategory : filterCategories) {
                    ...
                    final int filterProjectID = filterProject == null ? Constants.COMMON_PROJECT_ID : filterCategory.getProject()
                            .getProjectId();
                    ...
                    if (filterCategoryID == catKey && filterProjectID == projKey) {
                         ...
                         */
                        if ((filterCategory.getCategoryState() == CommonFilterConstants.CATEGORY_INTERNAL_AND_STATE || filterCategory
                                .getCategoryState() == CommonFilterConstants.CATEGORY_INTERNAL_OR_STATE)) {
                            if (filterCategory.getCategoryState() != dbIntCatLogic) filterCategory.setCategoryState(dbIntCatLogic);
                            foundIntCat = true;
                        }
                         ...
                         * does not match what the user just selected
                         */
                        if ((filterCategory.getCategoryState() == CommonFilterConstants.CATEGORY_EXTERNAL_AND_STATE || filterCategory
                                .getCategoryState() == CommonFilterConstants.CATEGORY_EXTERNAL_OR_STATE)) {
                            if (filterCategory.getCategoryState() != dbExtCatLogic) filterCategory.setCategoryState(dbExtCatLogic);
                            foundExtCat = true;
                        }
                    }
                     ...
                     */
                    if (foundIntCat && foundExtCat) break;
                }
                ...
                if (!foundIntCat || !foundExtCat) {
                    ...
                    if (!foundIntCat) createFilterCategory(filter, projectEntity, categoryEntity, dbIntCatLogic);
                    if (!foundExtCat) createFilterCategory(filter, projectEntity, categoryEntity, dbExtCatLogic);
                }
            }
        }
    }
