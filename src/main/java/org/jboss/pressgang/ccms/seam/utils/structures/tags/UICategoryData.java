...
public class UICategoryData extends UITagDataBase implements Comparable<UICategoryData> {
    ...
    public UICategoryData(final String name, final String description, final Integer id, final Integer sort, final boolean selected,
            final boolean notSelected, final boolean groupBy, final boolean mutuallyExclusive) {
        super(name, description, id, sort, selected, notSelected, groupBy);
        this.mutuallyExclusive = mutuallyExclusive;
    }

    public UICategoryData(final String name, final String description, final Integer id, final Integer sort,
            final boolean mutuallyExclusive) {
        super(name, description, id, sort, false, false, false);
        this.mutuallyExclusive = mutuallyExclusive;
        this.internalLogic = CommonFilterConstants.OR_LOGIC;
        this.externalLogic = CommonFilterConstants.AND_LOGIC;
    }
